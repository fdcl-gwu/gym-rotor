...
class EquivWrapper(QuadEnv):
    ...
    def reward_wrapper(self, obs):
        ...
        self.eIX_actual.integrate(eX_actual, self.dt) # eX + eV
        self.eIX_actual.error = np.clip(self.eIX_actual.error, -self.sat_sigma, self.sat_sigma)
        self.eIR_actual.integrate(eR_actual, self.dt) # eR + eW
        ...
        self.eIX_equiv.integrate(eX_equiv, self.dt) # eX + eV
        self.eIX_equiv.error = np.clip(self.eIX_equiv.error, -self.sat_sigma, self.sat_sigma)
        self.eIR_equiv.integrate(eR_equiv, self.dt) # eR + eW
        ...
        satisfy_equiv = bool(
            ...
            # and abs((linalg.norm(self.eIX_actual.error, 2) - linalg.norm(self.eIX.error, 2))) < 1e-7 # eIX errors
