...
from algos.utils.replay_buffer import ReplayBuffer
...
class Learner:
    def __init__(self, args):
        ...
        if self.test_model:
            total_steps, agent_id = 1040_000, 0  # edit 'total_steps' accordingly
    ...
    def train_policy(self):
        ...
        for self.total_timesteps in range(int(self.max_timesteps)):
            ...
            prev_act = copy.deepcopy(action) # previous action; a(t-1)
            ...
            if self.total_timesteps > self.start_timesteps:
                ...
                for agent_id in range(self.N):
                    self.agent_n[agent_id].train(self.replay_buffer, self.env)
    ...
    def eval_policy(self):
        ...
        for num_eval in range(self.num_eval):
            ...
            mode = 5 if self.test_model == True else 0
            """ Mode List -----------------------------------------------
            ...
            for _ in range(int(self.eval_max_steps)):
                ...
                if self.rl_algo == 'TD3':
                    ...
                    act_n = [agent.choose_action(obs, is_eval=True) for agent, obs in zip(self.agent_n, error_obs_n)] # obs_n
                action = np.concatenate((act_n), axis=None)
